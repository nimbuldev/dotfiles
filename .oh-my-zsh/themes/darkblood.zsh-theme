...
# ...One day.
THEME_PREFIX="%{$fg[green]%}[%{$fg_bold[white]%}"
THEME_SUFFIX="%{$reset_color%}%{$fg[green]%}]"
...
username_and_host(){
    ...
    echo -n "%n%{$reset_color%}%{$fg[green]%}@%{$fg_bold[red]%}%m"
}

git_stuffs(){
    ## Enter our git prompt
    echo -n "%{$(git_prompt_info)%}%(?,,%{$fg[green]%}[%{$fg_bold[white]%}%?%{$reset_color%}%{$fg[green]%}])"
}
...
pointy_arrow(){
    ...
    ## I have left the reset color after this little guy since we don't want our text being cologreen.
    echo -n "%{$fg[green]%}> "
}
...
ZSH_THEME_GIT_PROMPT_PREFIX="%{$fg[green]%}[%{$fg_bold[red]%}"
